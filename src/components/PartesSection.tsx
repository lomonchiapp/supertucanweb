...
const CATEGORIES = [
  {
    id: 'motor',
    name: 'partes.categories.motor',
    count: 45,
    icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z',
  },
  { id: 'carroceria', name: 'partes.categories.carroceria', count: 32, icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
  { id: 'electrico', name: 'partes.categories.electrico', count: 28, icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
  { id: 'frenos', name: 'partes.categories.frenos', count: 19, icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
  { id: 'suspension', name: 'partes.categories.suspension', count: 15, icon: 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z' },
  { id: 'accesorios', name: 'partes.categories.accesorios', count: 67, icon: 'M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z' },
];

const PARTS = [
  { id: 1, name: 'partes.parts.filtroAceite', category: 'motor', model: 'ADRI SPORT / BWS / CG200', price: 25.99, originalPrice: 32.99, inStock: true, discount: 20 },
  { id: 2, name: 'partes.parts.pastillasFreno', category: 'frenos', model: 'ADRI SPORT', price: 45.99, originalPrice: null, inStock: true, discount: 0 },
  { id: 3, name: 'partes.parts.carenado', category: 'carroceria', model: 'BWS', price: 89.99, originalPrice: 109.99, inStock: false, discount: 18 },
  { id: 4, name: 'partes.parts.bombilloLed', category: 'electrico', model: 'partes.parts.universal', price: 15.99, originalPrice: null, inStock: true, discount: 0 },
  { id: 5, name: 'partes.parts.kitCadena', category: 'motor', model: 'CG200 / ST 125', price: 67.5, originalPrice: 79.99, inStock: true, discount: 15 },
  { id: 6, name: 'partes.parts.amortiguador', category: 'suspension', model: 'ADRI SPORT', price: 120.0, originalPrice: null, inStock: true, discount: 0 },
];
...
export function PartesSection() {
  ...
  const activeCatNameKey = CATEGORIES.find((c) => c.id === activeCategory)?.name || 'partes.categories.all';

  const openWhatsApp = (partName: string) => {
    const msg = encodeURIComponent(t('partes.whatsapp.interested', { name: partName }));
    window.open(`https://wa.me/18092468383?text=${msg}`, '_blank');
  };
          ...
          <div className="max-w-2xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-[11px] font-bold tracking-[0.3em] text-[var(--color-primary)] font-accent">
                {t('partes.hero.eyebrow')}
              </span>
            </div>
            <h1 className="font-display text-5xl md:text-7xl font-bold text-neutral-900 tracking-tight uppercase leading-[0.9]">
              {t('partes.hero.title')}
              <br />
              <span className="text-[var(--color-primary)]">{t('partes.hero.titleAccent')}</span>
            </h1>
            <p className="text-neutral-600 text-base font-sans mt-5 leading-relaxed max-w-lg">
              {t('partes.hero.description')}
            </p>

            {/* Badges */}
            <div className="flex flex-wrap gap-4 mt-6">
              {[t('partes.badges.original'), t('partes.badges.warranty'), t('partes.badges.fastShipping')].map((badge) => (
                <div
                  key={badge}
            ...
            <div className="flex gap-10 mt-10">
              {[
                { val: '200+', label: t('partes.stats.parts') },
                { val: '12', label: t('partes.stats.countries') },
                { val: '24H', label: t('partes.stats.dispatch') },
              ].map((s) => (
                <div key={s.label}>
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mt-10 mb-6 flex-wrap gap-3">
          <h2 className="font-display text-3xl font-bold text-neutral-900 tracking-tight uppercase">
            {activeCategory === 'todos' ? t('partes.list.featuredTitle') : t(activeCatNameKey)}
          </h2>
          <span className="text-xs text-neutral-500 font-accent tracking-[0.15em] font-bold">
            {filtered.length} {filtered.length !== 1 ? t('partes.list.countPlural') : t('partes.list.countSingular')}
          </span>
        </div>
                  ...
                  <div className="flex-1 p-4 flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-[10px] text-neutral-500 font-accent tracking-[0.1em] mt-1 font-bold">
                        {part.model.startsWith('partes.') ? t(part.model) : part.model}
                      </p>
                    </div>
                    ...
                    <div className="flex items-end justify-between mt-3">
                      ...
                      >
                        {part.inStock ? t('partes.list.consult') : t('partes.list.outOfStock')}
                      </button>
                    </div>
                  </div>
                </div>
      ...
      <div className="relative bg-neutral-900 text-white py-20 lg:py-24 overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto px-6 lg:px-8 text-center">
          ...
          <p className="text-neutral-300 font-sans mb-10 text-base lg:text-lg">
            {t('partes.cta.description')}
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button
              onClick={() => {
                const msg = encodeURIComponent(t('partes.whatsapp.needHelp'));
                window.open(`https://wa.me/18092468383?text=${msg}`, '_blank');
              }}
      ...
      {cartItems.length > 0 && (
        <div className="fixed bottom-6 right-6 z-50">
          ...
          >
            ...
            {cartItems.length} {cartItems.length > 1 ? t('partes.cart.consultedPlural') : t('partes.cart.consultedSingular')}
          </div>
        </div>
      )}
    </div>
